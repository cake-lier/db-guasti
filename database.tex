...
mentre i progettisti sono interessati ad analizzare i dati dei guasti di una o più \textbf{categorie} di prodotto. Un tecnico inoltre può essere
...
e in caso l'insieme dei prodotti che presentano una stessa categoria dovrebbe essere rimosso, non è problematico. Infatti, significherebbe semplicemente
...
concetto di "guasto" tra essi ed il concetto di "intervento". È infatti facile immaginare come un cliente potrebbe voler riportare la presenza
di un guasto solamente dopo che un secondo si è verificato, magari perchè non era così urgente, e lo farebbe all'interno dello stesso intervento.
...
un prodotto caratterizzato anche da più guasti, infatti, potrebbero essere più di uno i "difetti" legati ad un "prodotto", ma questo impedirebbe
la loro ripetizione, mentre invece è assai probabile che se un "guasto" non viene correttamente riparato, sullo stesso "prodotto" si verificherà
di nuovo in futuro. Magari, più semplicemente, potrebbe anche verificarsi anche un guasto diverso, ma la cui origine è sempre lo stesso "difetto" di
progettazione. I dati che sono di competenza del tecnico inserire sono tutti modellati mediante attributi opzionali, poichè si suppone che
l'apertura della pratica coincida con l'inserimento di un nuovo \textit{record} nel \textit{database}, momento nel quale questi dati sono ancora
sconosciuti.
